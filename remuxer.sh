...
if [ "$#" -ge "2" ]; then
 if [ "$1" == "-g" ]; then
  ...
  if [ ! -x /usr/bin/ffmpeg ]; then echo "Please install ffmpeg first"
   ...
   video_scale_z='720'
   segment_seconds='60'
   audio_args=' -c:a ac3 -b:a 320K -ar 48000 '
   ...
   if [ ! -z "$(ffprobe -v 0 -show_entries format=duration -of compact "$name_m" | sed 's|[/|=A-Za-z]||g;s|0.040000||')" ]; then
    ...
    if [ "$2" == "a" ]; then
     ...
     printf "$split_args$audio_convert$audio_args:scq:Audio/$name_dir/$(if [ "$4" == "s" ]; then echo "$name_s"; fi)$splits$name_s.m4a:scq:"
    elif [ "$2" == "v" ]; then
 ...
 echo "Usage: ./remuxer.sh option type mode [extra options] [language]
  * Options
